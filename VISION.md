...
| 12 | Community layer to store/share spot info | ⬜ design sketched 2026-07-24 (from Tim's Chevron dump/water find, row 61-62 session): static-first per the architecture section — community spots live as versioned GeoJSON in a public repo, merged into the Sites layer as `src: community`. In-app "Report a spot" (dump, water, camp…) posts to a zero-server moderated queue (tiny Cloudflare Worker or GitHub-backed form; Tim approves; pipeline merges + deploys). Validation iOverlander-style: submissions start *unreported/unverified*, and check-ins ("worked for me" / "gone", dated) accumulate — 2+ independent confirmations promote to verified, and the card always shows last-confirmed age so staleness is honest. Needs Tim's calls: submission channel, moderation flow |
| 13 | Rig/setup profile (RV, tent, overlander…) → "can my rig get there?" | ⬜ |
| 14 | Navigate to a spot via Google Maps / Apple Maps handoff | ✅ shipped 2026-07-12 as row 37 — Apple/Google deep links on every point card |
