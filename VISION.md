...
| 90 | Navigate to a point with the compass: pick a target and get straight-line guidance — a line on the map to the point, distance, and the compass ribbon showing which way to turn | 2026-07-28 |  | ⬜ builds directly on row 89's heading hook and cluster; beeline guidance first, as the simplest honest mode |
...
| 95 | Full-screen instrument mode — a standalone compass + speed + elevation (+ optional coordinates) screen, "like a handheld compass" and nothing else | 2026-07-28 |  | ⬜ Tim's request 2026-07-28. A dedicated full-screen readout that works in **both landscape and portrait**, styled to look cool. Feeds off row 89's live sensor hook but stripped to just the instruments. Wants: adjustable speed-readout size; a chooser for compass style (numbers / dial / forward-facing, etc.); ability to punch in a cardinal/target bearing to steer toward; and an on/off toggle for **each** element independently (compass, speed, elevation, coordinates). Overlaps row 90 (bearing-to-target) on the "punch in a direction" part |
