...
- **Sun path:** where the sun tracks across the sky from a spot on a day, from the toolbar's sun icon or **Sun & shade here** on any point card — as a sun-path dome anywhere, or drawn over the camera in AR on a phone, with the time scrubbable and the date steppable. **Scan ridgeline** reads the app's own elevation data around the point and reports when the sun actually clears the terrain, how much direct sun the spot gets, what the ridge costs against an open site, and the panel tilt and bearing that collect the most. Solar position follows the NOAA equations and is checked against MET Norway and the US Naval Observatory in `boondock/src/shared/sun.test.mjs`
- **Sight a point:** aim the phone's camera at something you can see — a forest road cut, a meadow, a saddle — put the crosshair on it, and the app estimates where that point is by walking the line of sight through its own elevation data until it meets the ground (the fire lookout's method, with the terrain model as the second tower). Compass readings are corrected from magnetic to true north with the embedded World Magnetic Model, a one-tap **Align on the sun** measures out your phone's own compass bias, and the map draws the strip of ground your aim error actually spans — never just a confident pin. Manual mode takes a typed true bearing instead (desktop, or a bearing read off a real compass)
...
- [NOAA NCEI & British Geological Survey](https://www.ncei.noaa.gov/products/world-magnetic-model) — World Magnetic Model 2025 coefficients (public domain), embedded for magnetic-declination correction
