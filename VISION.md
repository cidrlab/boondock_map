...
| 89 | Live instrument cluster from the phone's own sensors — compass, speed, and elevation readouts, "live, based on the location, direction, and elevation of the phone", with a horizontal compass ribbon like Gaia's trip bar but styled as our own design; shown/hidden by its own toggle button (Tim's call, 2026-07-27). In-app counterpart to row 38's CarPlay idea | 2026-07-27 |  | ⬜ scoped: one glass card top-center (elevation + speed cells over a sliding compass tape with honest `mag`/`gps` source tags), toggle in the bottom-right control stack; layout cue from Gaia GPS, credited per the attribution policy |
